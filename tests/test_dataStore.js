...
// ── Helpers for importFromFile tests ─────────────────────────────────────
...
describe('DataStore — importFromFile', () => {
  ...
  beforeEach(() => {
    MOCK_USERDATA = fs.mkdtempSync(path.join(os.tmpdir(), 'mtg-ds-'));
    tmpFile       = path.join(os.tmpdir(), `mtg-backup-${Date.now()}.json`);
    ds = new DataStore();
  });
  ...
  test('imports matches from export file', () => {
    writeExportFile(tmpFile, { matches: [{ id: 'm1', result: 'win' }] });
  ...
  test('skips duplicate matches already present', () => {
    ...
    writeExportFile(tmpFile, {
      matches: [{ id: 'm1', result: 'win' }, { id: 'm2', result: 'loss' }],
    });
    ds.importFromFile(tmpFile);
    expect(ds.getMatches()).toHaveLength(2);
  });

  test('imports decks from export file', () => {
    writeExportFile(tmpFile, { decks: { 'Mono Red': { name: 'Mono Red' } } });
    ds.importFromFile(tmpFile);
  ...
  test('imports multiple matches, all present after import', () => {
    writeExportFile(tmpFile, {
      matches: [
        { id: 'm1', result: 'win',  timestamp: '2026-01-01T00:00:00Z' },
        { id: 'm2', result: 'loss', timestamp: '2026-01-02T00:00:00Z' },
        { id: 'm3', result: 'win',  timestamp: '2026-01-03T00:00:00Z' },
      ],
    });
    ds.importFromFile(tmpFile);
    const ids = new Set(ds.getMatches().map(m => m.id));
    expect(ids).toEqual(new Set(['m1', 'm2', 'm3']));
  });

  test('does not duplicate existing decks (import wins on key collision)', () => {
    ds.data.decks['existing'] = { name: 'Original' };
    ds.saveData();
    writeExportFile(tmpFile, { decks: { 'existing': { name: 'Updated' }, 'new-deck': { name: 'New' } } });
    ds.importFromFile(tmpFile);
    // spread means import overwrites on collision — both decks present
    expect(ds.getDeck('existing')).not.toBeNull();
    expect(ds.getDeck('new-deck')).not.toBeNull();
  });
